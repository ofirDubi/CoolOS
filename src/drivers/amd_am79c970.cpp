...
: Driver(),                              
    ...
    registerDataPort(dev->IOBARAddress + 0x10),
    registerAddressPort(dev->IOBARAddress + 0x12),
    resetPort(dev->IOBARAddress + 0x14),
    busControlRegisterDataPort(dev->IOBARAddress + 0x16)
...
uint32_t amd_am79c970::HandleInterrupt(uint32_t esp){
   ...
   // printf("INTERRUPT FROM AMD am79c970\n");
    ...
    CHECK_DEVICE_CODE(temp, 0x8000,"AMD am79c970 ERROR\n" );
    CHECK_DEVICE_CODE(temp, 0x2000, "AMD am79c970 COLLISION ERROR\n");
    CHECK_DEVICE_CODE(temp, 0x1000,"AMD am79c970 MISSED FRAME ERROR\n" );
    CHECK_DEVICE_CODE(temp, 0x0800,"AMD am79c970 MISSED MEMORY ERROR\n");
    if((temp & 0x0400) == 0x0400) Receive(); //DATA RECEIVED
    CHECK_DEVICE_CODE(temp, 0x0200,"AMD am79c970 DATA SENT\n" );
    ...
    CHECK_DEVICE_CODE(temp, 0x0100, "AMD am79c970 INIT DONE\n");
...
void amd_am79c970::Send(uint8_t* buffer, int size){
   ...
   // printf("AMD am79c970 SENDING DATA");
    ...
    if(size > 1518){
        ...
        printf("AMD am79c970 WARNING: data is too big, sending the first 1518 bits.\n");
        size = 1518;
    }
